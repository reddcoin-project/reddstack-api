...
def make_default_config(path=CONFIG_PATH):
    ...
    if not os.path.exists(path):
        ...
        parser.add_section('reddstore-client')
        parser.set('reddstore-client', 'server', BLOCKSTORED_SERVER)
        parser.set('reddstore-client', 'port', BLOCKSTORED_PORT)
        parser.set('reddstore-client', 'metadata', BLOCKSTORE_METADATA_DIR)
        parser.set('reddstore-client', 'storage_drivers', BLOCKSTORE_DEFAULT_STORAGE_DRIVERS)
        parser.set('reddstore-client', 'blockchain_headers', SPV_HEADERS_PATH)
        parser.set('reddstore-client', 'advanced_mode', 'false')
...
def get_config(path=CONFIG_PATH):
    ...
    if parser.has_section("reddstore-client"):

        # blockstored
        if parser.has_option("reddstore-client", "server"):
            config['server'] = parser.get("reddstore-client", "server")

        if parser.has_option("reddstore-client", "port"):
            try:
                config['port'] = int(parser.get("reddstore-client", "port"))
            except:
                log.error("Invalid 'port=' setting.  Please use an integer")

        if parser.has_option("reddstore-client", "storage"):
            config['storage_drivers'] = parser.get("reddstore-client", "storage")

        if parser.has_option("reddstore-client", "metadata"):
            config['metadata'] = parser.get("reddstore-client", "metadata")

        if parser.has_option("reddstore-client", "blockchain_headers"):
            config['blockchain_headers'] = parser.get("reddstore-client", "blockchain_headers")

        if parser.has_option("reddstore-client", "advanced_mode"):
            config['advanced_mode'] = parser.get("reddstore-client", "advanced_mode")
